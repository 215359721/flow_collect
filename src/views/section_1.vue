...
export default {
  ...
  mounted () {
    ...
    // this.sourceData = this.initData(realData)
  ...
  methods: {
    ...
    initData (data) {
      data.nodes.forEach((element) => {
        // element.type = 'image'
        element.img = require('../assets/icon/' + element.icon + '.png')
        element.obj = {
    ...
    showTypeChange (val) {
      this.showType = val
      this.sourceData = this.initData(this.sourceData)
      this.graph.changeData(this.sourceData)
      this.changeLayout(this.curLayout)
    },
    /**
     * 数据源切换
     */
